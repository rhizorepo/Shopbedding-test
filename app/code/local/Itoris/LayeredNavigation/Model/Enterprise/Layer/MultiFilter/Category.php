 ...
 */
class Itoris_LayeredNavigation_Model_Enterprise_Layer_MultiFilter_Category extends Enterprise_Search_Model_Catalog_Layer_Filter_Category {
	 ...
	 * @param $categories
	 */
	protected function updateCategoryFilter($categories) {

		$productCollection = $this->getLayer()->getProductCollection();
		if (version_compare(Mage::getVersion(), '1.12.0.0', '<')) {
			$productCollection->addFqFilter(array('categories' => $categories));
		} else {
			$productCollection->addFqFilter(array('category_ids' => $categories));
		}
	}
