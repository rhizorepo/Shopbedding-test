...
class Mage_Catalog_Model_Product_Attribute_Backend_Media extends Mage_Eav_Model_Entity_Attribute_Backend_Abstract
{
     ...
     * @return string
     */
    public function addImage(Mage_Catalog_Model_Product $product, $file, $mediaAttribute=null, $move=false, $exclude=true, $label=null, $sort=null, $swatch_image=null, $swatch_image_label=null, $isCategory = 1)
    {
        $file = realpath($file);
        Mage::log("image upload: ".$file);
        ...
        //Added to allow sorting from import
        if (isset($sort) && !empty($sort)){
            $position = $sort;
        }

        //Modified to set label
        $mediaGalleryData['images'][] = array(
            'file'     => $fileName,
            'position' => $position,
            'label'    => $label,
            'swatchimage'    => $swatch_image,
            'swatchimage_label'    => $swatch_image_label,
