    ...
    extends Mage_Eav_Model_Convert_Parser_Abstract
{
    ...
    public function unparse()
    {
		...
		$custom_profile = 12;
		$excluded_array = $this -> getCsvContent();

		$profile_id = Mage::registry('current_convert_profile')->getId();

		//$product_inst = $this->getProductModel()->setStoreId($this->getStoreId())->load($entityId);
		  ...
		  foreach ($entityIds as $i => $entityId) {
			...
			/*========================================================================*/
			//Mage::log($product -> getTypeID(), null, 'custom.log');

			if($profile_id == $custom_profile){
				if($product -> getTypeId() =="simple"){
					$simpleProductId = $entityId;
					$config_parentIds = Mage::getResourceSingleton('catalog/product_type_configurable')
										->getParentIdsByChild($simpleProductId);

					if($config_parentIds){
						$config_product = Mage::getModel('catalog/product')->load($config_parentIds[0]);

						if($config_product -> getStatus() !=1){
							continue;
						}
						else{
							//Mage::log($config_product -> getVisibility()."--".$config_product -> getSku(), null, 'custom.log');
							//================================
							if($config_product -> getVisibility() ==4){
								$parentIds = $config_parentIds;
							}
							if($config_product -> getVisibility() ==1){
								$config_product_id = $config_product -> getId();
								$group_parentIds = Mage::getSingleton('catalog/product_type_grouped')->getParentIdsByChild($config_product_id);
								if($group_parentIds){
									$group_product = Mage::getModel('catalog/product')->load($group_parentIds[0]);
									if($group_product -> getVisibility() !=1){
										$parentIds = $group_parentIds;
									}
								}
			...
			$product_url = "";
				...
				if (!array_key_exists($entityId, $excluded_array)) {

					if(isset($parentIds)){
						$parent_id = $parentIds[0];
						$product1 = Mage::getModel('catalog/product')->load($parent_id);

						if (array_key_exists($parentIds[0], $excluded_array)) {
							$product_url = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB).$excluded_array[$parent_id] .".html";
						}
						else{
							$idPath = sprintf('product/%d', $product1->getId());
							$rewrite = Mage::getModel('core/url_rewrite')->loadByIdPath($idPath);
							$product_url =  Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB).$rewrite->getRequestPath();
						}
					}
					else{
					...
					$product_url = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB).$excluded_array[$entityId] .".html";
				}
				 $row = array(
	...
	public function getCsvContent(){
		//$path = "/home/shopbedd/shopbedding.com/html/exclude.csv";
		$path = Mage::getBaseDir()."/exclude.csv";
		$exclude_id_arr = array();
		$file = fopen($path,"r");
		while(! feof($file)){
			$exclude_id_arr[] = fgetcsv($file);
		}
		fclose($file);
		$index = 0;
		foreach($exclude_id_arr as $exclude_row){
			if($index != 0){
			$key_val = $exclude_row[4];
				$exclude_id[$key_val] = $exclude_row[1];
			}
			$index++;
		}
		return $exclude_id;
