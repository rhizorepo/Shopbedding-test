    ...
    extends Mage_Eav_Model_Convert_Adapter_Entity
{
    ...
    public function saveRow(array $importData)
    {
        ...
        $_images = $product->toArray();
        if(array_key_exists('media_gallery', $_images)) {
            $_images = $_images['media_gallery']['images'];
            $_empty_product = false;
            //Mage::log('!empty product',null,'images.log');
        } else {
            $_empty_product = true;
            //Mage::log('empty product',null,'images.log');
        }

        if(!empty($importData['grouped_big_swatch_image'])){
            $swatch_images = explode(';',$importData['grouped_big_swatch_image']);
            if(is_array($swatch_images)){
                $swatch_labels = explode(';',$importData['grouped_big_swatch_image_label']);
                $swatch_small_images =  explode(';',$importData['grouped_swatch_image']);
                $swatch_small_labels =  explode(';',$importData['grouped_swatch_image_label']);
                try {
                    foreach($swatch_images as $key=>$swatch_image){
                        if($_empty_product){
                            //Mage::log('empty product mode',null,'images.log');
                            $img_path = Mage::getBaseDir('media') . DS . 'import'  . trim($swatch_image);
                            $product->addImageToMediaGallery($img_path,'image',false, true,$swatch_labels[$key],null,$swatch_small_images[$key],$swatch_small_labels[$key], 1);
                        }elseif(!$this->ifImageExists($_images, $swatch_image)){
                            //Mage::log('prodcut exitsts and check passed',null,'images.log');
                            $img_path = Mage::getBaseDir('media') . DS . 'import'  . trim($swatch_image);
                            $product->addImageToMediaGallery($img_path,'image',false, true,$swatch_labels[$key],null,$swatch_small_images[$key],$swatch_small_labels[$key], 1);
                        }else{
                            //Mage::log('nothing',null,'images.log');
                        }
                    }
                }
                catch (Exception $e){}
        ...
        foreach ($imageData as $file => $fields) {
            try {
//                    Mage::log($imageData, null, 'images.log');
                    if(!$_empty_product){
                        if(!$this->ifImageExists($_images, $file)) {
    ...
    private function ifImageExists($image_array, $image_needle){
        if(!is_array($image_array) || empty($image_array))
            return ;
        $image_needle = substr($image_needle,0,strpos($image_needle,'.jpg'));
//        Mage::log($image_array,null,'images.log',true);
        foreach($image_array as $image){
             if(strpos($image['file'],$image_needle) !== false){
//                 Mage::log('check return true',null,'images.log');
                 return true;
             }
        }
//        Mage::log('check return false',null,'images.log');
        return false;
    }
