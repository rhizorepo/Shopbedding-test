...
class Mage_Catalog_Block_Product_View extends Mage_Catalog_Block_Product_Abstract
{
    ...
    public function getCustomTabs()
    {
        ...
        // requires the addition of a product attribute called "custom_tabs" which is a
        // text field, not unique, not required, no validation for all product types.
        // It should be added to all products, and can be associated with any attribute
        // set.


        $product = $this->getProduct();
        $customtabsstr = $product->getCustomTabs();
        $cmsitemsarray = explode(",",$customtabsstr);
        ...
        if ( $cmsitemsarray )
        {
            if ( count( $cmsitemsarray ) > 0 )
            {
                foreach ($cmsitemsarray as $itemname)
                {
                    ...
                    if ( $itemname == "description" ) {
                        ...
                        $tabs["Description"] = $product->getDescription();
                        
                    } else if ( strlen( $itemname ) > 0 ) {

                        $cmsblock = Mage::getModel('cms/block')->setStoreId($store_id);
                        $cmsblock->load($itemname);
                        // ->getContent();

                        // Mage::log($cmsblock->getTitle());
                        // Mage::log($cmsblock->getContent());

                        // Mage::log(print_r( $itemname,1 ));
                        // Mage::log(print_r( $cmsblock, 1));
        ...
        // Mage::log(print_r( $tabs,1 ));

        // return print_r($cmsitemsarray,1);
        return $tabs;
