...
class Mage_Core_Controller_Varien_Router_Standard extends Mage_Core_Controller_Varien_Router_Abstract
{
    ...
    protected function _checkShouldBeSecure($request, $path = '')
    {
        ...
        /** Start code override */
        $alias = ltrim($request->getAlias(Mage_Core_Model_Url_Rewrite::REWRITE_REQUEST_PATH_ALIAS), '/');
        $urls  = Mage::getConfig()->getNode('default/unsecure_url')->asArray();
        $unsecureUrls = array();
        foreach ($urls as $url) {
            $unsecureUrls[] = ltrim($url, '/');
        }
        if (!$this->_shouldBeSecure($path) && in_array($alias, $unsecureUrls) && Mage::app()->getStore()->isCurrentlySecure()) {
