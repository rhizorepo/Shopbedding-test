...
class Mirasvit_FpcCrawler_Block_Adminhtml_Crawlerlogged_Url_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    {
        $this->_sortByPageTypeCount = count(Mage::getSingleton('fpccrawler/config')->getSortByPageType(true));
        $this->_sortByProductAttributeCount = count(Mage::getSingleton('fpccrawler/config')->getSortByProductAttribute(true));
        $this->_sortCrawlerUrls = Mage::getSingleton('fpccrawler/config')->getSortCrawlerUrls(true);

        parent::__construct();
        $this->setId('grid');
        if (Mage::getSingleton('fpccrawler/config')->getSortCrawlerUrls(true) == 'popularity') {
    ...
    protected function _setCollectionOrder($column)
    {
        ...
        if ($collection) {
            ...
            if ($this->_sortCrawlerUrls == 'popularity'
                ...
                $collection->getSelect()->order(Mage::helper('fpccrawler')->getOrderSql(true, true));
            } elseif ($this->_sortCrawlerUrls == 'custom_order' && $this->_sortByPageTypeCount > 0) {
                $collection->getSelect()->order(Mage::helper('fpccrawler')->getOrderSql(false, true));
    ...
    protected function _prepareCollection()
    {
        ...
        $collection = Mage::getModel('fpccrawler/crawlerlogged_url')
            ->getCollection();
    ...
    protected function _customerGroupFilter($collection, $column)
    {
        ...
        foreach ($collection as $item) {
            if (strpos(strtolower($item->getCustomerGroupCode()), strtolower($value)) !== false) {
                $customerGroupIds[] = $item->getCustomerGroupId();
            }
        }

        $this->getCollection()->getSelect()->where("customer_group_id IN(?)", $customerGroupIds);
