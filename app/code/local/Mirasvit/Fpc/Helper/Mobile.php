...
class Mirasvit_Fpc_Helper_Mobile extends Mage_Core_Helper_Abstract
{
    /**
     * @return bool|string
     */
    public function getMobileDeviceType()
    {
        ...
        if (Mage::getSingleton('fpc/config')->getMobileDetect()) {
            $detect = new Mobile_Detect_Extended;
            $deviceType = $detect->isMobile()
                ? ($detect->isTablet()
                    ? 'tablet'
                    : Mirasvit_FpcCrawler_Model_Config::MOBILE_GROUP)
                : Mirasvit_FpcCrawler_Model_Config::COMPUTER_GROUP;

            if ($deviceType == 'tablet') { // use the same cache for desktop and tablet
                $deviceType = Mirasvit_FpcCrawler_Model_Config::COMPUTER_GROUP;
            }
        }
