...
class Amasty_Rules_Model_Observer
{
    ...
    protected function _getQty($rule, $cartQty)
    {
		...
		//modifed by cw
		if($rule->getSimpleAction() == 'buy_x_get_y_percent') {
			...
			foreach ($items as $item) {      
				$totalQty[$item->getSku()] = $this->_getItemQty($item);
			}
			foreach($skus as $sku){
				if(array_key_exists(trim($sku), $totalQty)){
					$promoQty +=$totalQty[trim($sku)];
				}
			}
			return min($promoQty, $rule->getDiscountQty());
		} else {
			return $discountQty;
		}	
		//modifed by cw
