    ...
    protected function _createObjects($params, $objects = array())
    {
        ...
        if (isset($objects['order'])) {
            ...
            foreach ($objects['order']->getAllVisibleItems() as $item) {
                ...
                // HACK for simple product with 'Not Visible' status (compatibility with SimpleConfigurableOroduct)
                
                if ($item->getProductType() == 'simple' &&  is_callable(array($item->getProduct()->getTypeInstance(), 'hasConfigurableProductParentId'))) {
                   $parentIds = Mage::getModel('catalog/product_type_configurable')->getParentIdsByChild($item->getProductId());
                   empty($parentIds) ? $productId = $item->getProductId() : $productId = $parentIds[0];
                } else {
        ...
        if (isset($objects['quote'])) {
            if (!$objects['quote']->getId()) {
                ...
                foreach ($objects['quote']->getAllVisibleItems() as $item) {
                    $product = Mage::getModel('catalog/product')->setStoreId($item->getStoreId());
                    // HACK for simple product with 'Not Visible' status (compatibility with SimpleConfigurableProduct)
		      if ($item->getProductType() == 'simple' &&  is_callable(array($item->getProduct()->getTypeInstance(), 'hasConfigurableProductParentId'))) {
			$parentIds = Mage::getModel('catalog/product_type_configurable')->getParentIdsByChild($item->getProductId());
			empty($parentIds) ? $productId = $item->getProductId() : $productId = $parentIds[0];
		      } else {
