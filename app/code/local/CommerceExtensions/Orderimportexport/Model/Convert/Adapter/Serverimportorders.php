...
class CommerceExtensions_Orderimportexport_Model_Convert_Adapter_Serverimportorders
...
//        }
	...
	public function	getOrderByIncrement($incrementId){
		...
		$result=$write->query("SELECT entity_id FROM `sales_flat_order_grid` WHERE `increment_id` = '".$incrementId."' ");
		$row = $result->fetch();
		if($row['entity_id']){
...

	{
		...
		if($this->getOrderByIncrement($importData['order_id'])){
			return;
		}
		#print_r($importData);
		...
		// CODE START FOR PROCESSING LINKED ORDERS ON TRANSFERS
		if (strpos($importData['order_id'], '-') !== FALSE) {
			
			$orderId = explode("-" , $importData['order_id']);
			$orders = Mage::getModel('sales/order')->getCollection();
			$orders->addFilter("increment_id", $orderId[0]);
	
            $oldOrder = $orders->getFirstItem();
            if ($oldOrder && $oldOrder->getId()) {
            	Mage::getSingleton('adminhtml/session_quote')->setOrderId($oldOrder->getId());
			} else {
				#return;
			}
		}	
        ...
        if ($orderId) {
            ...
            if ($ord && $ord->getId()) {
			    if (strcasecmp(trim($this->getBatchParams('update_orders')), 'true') == 0) {
					$updateAdapter = Mage::getModel('commerceextensions_orderimportexport/convert_adapter_updateorders');
					$updateAdapter->saveRow($importData);
					return;

				} else {
					Mage::log('Order #'.$importData['order_id'].' Already Exists row Skipped', null,'ce_order_import_errors.log');
        ...
        foreach ($products_ordered as $data) {
			...
			try {
					...
					Mage::log(sprintf('Order #'.$importData['order_id'].' ERROR PRODUCT DOES NOT EXIST: [sku]' . $product_id . ' %s', $e->getMessage()), null,'ce_order_import_errors.log');
		...
		if(isset($importData['is_guest'])) {
			if($importData['is_guest'] == 1) {
				...
				$street_s = array("0"=>$importData['shipping_street_full'], "1"=>isset($importData['shipping_street2']) ? $importData['shipping_street2'] : "", "2"=>isset($importData['shipping_street3']) ? $importData['shipping_street3'] : "", "3"=>isset($importData['shipping_street4']) ? $importData['shipping_street4'] : "");
...

    {
        ...
        if ($customer->getId()) {
			...
			if ($this->getBatchParams('update_customer_address') == "true") {
				...
				$street_b = array("0"=>$importData['billing_street_full'], "1"=>isset($importData['billing_street2']) ? $importData['billing_street2'] : "", "2"=>isset($importData['billing_street3']) ? $importData['billing_street3'] : "", "3"=>isset($importData['billing_street4']) ? $importData['billing_street4'] : "");
				...
				if ($defaultBillingId = $customer->getDefaultBilling()){
					 ...
					 $customerAddressBilling = Mage::getModel('customer/address');
					 $customerAddressBilling->setCustomerId($customer->getId())
									 ->setIsDefaultBilling('1')
									 ->setSaveInAddressBook('1');   
					 $customer->addAddress($customerAddressBilling);
					 try {
					 	$customerAddressBilling->addData($dataBilling)->save(); 
					 } catch(Exception $e){
				...
				if ($defaultShippingId = $customer->getDefaultShipping()){
					 ...
					 $customerAddressShipping = Mage::getModel('customer/address');
					 $customerAddressShipping->setCustomerId($customer->getId())
									 ->setIsDefaultShipping('1')
									 ->setSaveInAddressBook('1');   
					 $customer->addAddress($customerAddressShipping);
					 
					 try {
						$customerAddressShipping->addData($dataShipping)->save();           
					 } catch(Exception $e){
            ...
            $customerId = $this->_CreateCustomer($importData['password'], $importData['password_hash'], $importData['billing_company'], $importData['billing_city'], $importData['billing_telephone'], $importData['billing_fax'], $importData['email'], $importData['billing_prefix'], $importData['billing_firstname'], $importData['billing_middlename'], $importData['billing_lastname'], $importData['billing_suffix'], isset($importData['taxvat']) ? $importData['taxvat'] : "", $importData['billing_street_full'], isset($importData['billing_street2']) ? $importData['billing_street2'] : "", isset($importData['billing_street3']) ? $importData['billing_street3'] : "", isset($importData['billing_street4']) ? $importData['billing_street4'] : "", $importData['billing_postcode'], $importData['billing_region'], $importData['billing_country'], $importData['store_id'], $importData['shipping_prefix'], $importData['shipping_firstname'], $importData['shipping_middlename'], $importData['shipping_lastname'], $importData['shipping_suffix'], $importData['shipping_street_full'], isset($importData['shipping_street2']) ? $importData['shipping_street2'] : "", isset($importData['shipping_street3']) ? $importData['shipping_street3'] : "", isset($importData['shipping_street4']) ? $importData['shipping_street4'] : "", $importData['shipping_postcode'], $importData['shipping_region'], $importData['shipping_country'], $importData['shipping_city'], $importData['shipping_telephone'], $importData['shipping_fax'], $importData['shipping_company'], $valueid, $importData['is_subscribed'], $importData['group_id'], isset($importData['dob']) ? $importData['dob'] : "");
    ...
    public function assembleOrderData($customerId, $importData, $orderDataId, $add_products_array, $customer, $final_shipping)
    {
		...
		if (isset($importData['is_guest'])) {
			if ($importData['is_guest'] == 1) {
				...
				$regions = Mage::getResourceModel('directory/region_collection')->addRegionNameFilter($billing_region)->load();
				if ($regions) {
					 foreach($regions as $region) { $region = $region->getId(); }
				} else {
					 $region = $billing_region;
				}
				...
				if (method_exists($customer->getDefaultShippingAddress(), 'getEntityId')) {
					$billing_address_final = array(
        ...
        $orderData = array(
            ...
            'order' => array(
                ...
                'shipping_address' => array(
                    ...
                    'country_id' => $final_shipping['country_id'],
    ...
    { //Create a new Invoice for the order
        ...
        try {
            ...
            echo "Order # " . $order1->getId() . " failed to create an invoice: " . $e->getMessage();
            Mage::log(sprintf('Order # ' . $order1->getId() . ' failed to create an invoice: %s', $e->getMessage()), null, 'ce_order_import_errors.log');
			...
			//fix for 1.4.1.1
                ...
                if ($shippingMethod != 'flatrate_flatrate') {
                    ...
                    try {
                        ...
                        echo "Order #" . $importData['order_id'] . " ERROR: " . $e->getMessage();
		...
		$product2 = Mage::getModel('catalog/product')->loadByAttribute('sku', $parts[0]);
		 ...
		 if (method_exists($product2, 'getTypeId')) {
			...
			try {
            	...
            	if ($this->getBatchParams('use_historic_tax') == "true") {
        ...
        if (isset($importData['subtotal']) && isset($importData['shipping_amount'])) {
			...
			if ($verChecksplit[1] > 4) {
                ...
                try {
                    ...
                    echo "Order #" . $importData['order_id'] . " saving error: " . $e->getMessage();
                    Mage::log(sprintf('Order #' . $importData['order_id'] . ' saving error: %s', $e->getMessage()), null, 'ce_order_import_errors.log');
