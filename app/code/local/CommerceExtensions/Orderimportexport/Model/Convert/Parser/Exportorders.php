    ...
    extends Mage_Eav_Model_Convert_Parser_Abstract
{
    ...
    public function unparse()
    {
            	...
            	$payment_method=$this->getVar('filter_by_payment_method');
				$i="";
				...
				if($this->getVar('date_from') != "" && $this->getVar('date_to') != "" && $this->getVar('filter_by_order_status') != "" && $this->getVar('filter_by_store_id') != "") {
					...
					{
						$orders= $this->applyPaymentFilter($orders, $payment_method);
					}
					$orders->load();
		...
		*/
				...
				if ($order->getCustomerIsGuest()) {
					...
					if(method_exists($order, 'getBillingAddress') && method_exists($order->getBillingAddress(), 'getData')) {
						...
						$row['billing_street_full'] = $order->getStreet(1);
			...
			*/
			...
			if($this->getVar('export_product_tax_percent') == "true") {
		   ...
		   $select_qry = $read->query("SELECT tax_percent FROM `".$prefix."sales_flat_order_item` WHERE order_id ='".$order->getId()."' LIMIT 1");

                	$newrowItemId = $select_qry->fetch();
					$row['tax_percent'] = round($newrowItemId['tax_percent'],0);
				}
			}
