    ...
    extends Mage_Catalog_Model_Product_Type_Configurable
{
    ...
    protected $finalPriceCache = array();
    ...
    public function getUsedProducts($requiredAttributeIds = null, $product = null)
    {
        ...
        if (!$this->getProduct($product)->hasData($this->_usedProducts)) {
            ...
            foreach ($collection as $item) {
                $this->finalPriceCache[$item->getId()] = $item->getFinalPrice();
                $usedProducts[] = $item;
            }
    ...
    public function getFinalPriceForUsedProduct($productId) {
        if(isset($this->finalPriceCache[$productId])) {
            return $this->finalPriceCache[$productId];
        }
        return FALSE;
    }
