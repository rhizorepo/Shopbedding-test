...
class Ebizmarts_MageMonkey_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function getMergeVars($customer, $includeEmail = FALSE, $websiteId = NULL)
	{
		...
		$subscriber = $customer->getData();
		$subtotal = Mage::helper('checkout/cart')->getQuote()->getSubtotal();
		$fullActionName = Mage::app()->getFrontController()->getAction()->getFullActionName();

		if($request->getRouteName()=='customer'){
			 $merge_vars['SOURCE'] = 'Create Account';
		} else if($request->getRouteName()=='newsletter'){
			$merge_vars['SOURCE'] = 'signup1';
		} else if ($fullActionName == 'checkout_cart_index') {
			$merge_vars['SOURCE'] = 'Checkout';
		} else if ($fullActionName == 'customer_account_create') {
			$merge_vars['SOURCE'] = 'Create Account';
		} else if ($request->getRouteName()=='paypal') {
			$merge_vars['SOURCE'] = 'checkout';
		} else {
		     $merge_vars['SOURCE'] = Mage::app()->getRequest()->getRouteName();
		}
Mage::log($subscriber, null, 'mylogfile.log');
		if(isset($subscriber['subscriber_id'])){
			$merge_vars['DONTEMAIL'] = 'Y';
		}
		//GUEST
			...
			foreach($curlists as $listId => $list){
				...
				if(FALSE === array_key_exists($listId, $lists)){
					...
					if($defaultList == $listId){
                        ...
                        $this->unsubscribeDefaultList($email);
     ...
     * @return $this
     */
    public function unsubscribeDefaultList($email)
    {
